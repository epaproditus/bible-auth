...
function normalizeBookNameForMatch(name) {
  return normalizeText(name).toLowerCase().replace(/[^a-z0-9]/g, '')
}

function resolveCanonScope() {
  const value = normalizeText(process.env.RECOVERY_VERSION_CANON || 'new_testament').toLowerCase()
  if (value === 'all' || value === 'whole_bible' || value === 'full') return 'all'
  if (value === 'new' || value === 'nt' || value === 'new_testament') return 'new_testament'
  return 'new_testament'
}

function filterBooksByCanon(books) {
  const scope = resolveCanonScope()
  if (scope === 'all') return books

  return books.filter((book) => NEW_TESTAMENT_BOOKS.has(normalizeBookNameForMatch(book.name)))
}
...
export async function fetchRandomRecoveryVerse() {
  const books = filterBooksByCanon(await fetchBookIndex())
